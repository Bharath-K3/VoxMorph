"""
Wav2Vec2 Encoder Patch
-----------------------
Patches ChatterboxTTS to use Facebook's Wav2Vec2 speaker verification 

Input: 16kHz audio (Downsampled internally)
Output: 256-dim embedding (Strided from 768-dim)
"""
...
def apply_patch(model_instance, device: str):
    """
    Applies the Wav2Vec2 patch to the ChatterboxTTS model instance.
    """
    try:
        from transformers import Wav2Vec2Model, Wav2Vec2Processor
        print("[INFO] Loading Wav2Vec2-Base Encoder (768->256 Stride)...")
        ...
        model_instance.external_encoder_model = w2v_model
        model_instance.external_encoder_processor = w2v_processor
        
    except ImportError:
        print("[ERROR] Transformers not found. Install with: pip install transformers")
        raise

    def wav2vec2_prepare_conditionals(self, wav_fpath, exaggeration=0.5):
        ...
        # 3. Wav2Vec2 Embedding Extraction (16kHz)
        ...
        # 4. Dimension Matching (768 -> 256) - Stride Selection
        # Take every 3rd element (768 / 3 = 256)
        indices = torch.arange(0, 768, 3, device=device)[:256]
        emb = emb[..., indices]
    ...
    model_instance.prepare_conditionals = types.MethodType(wav2vec2_prepare_conditionals, model_instance)
    print("[INFO] Wav2Vec2 Encoder Patch Applied.")